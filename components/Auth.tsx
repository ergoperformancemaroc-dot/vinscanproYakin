...
const Auth: React.FC<AuthProps> = ({ users, onLogin, companyName, appName }) => {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white rounded-[3rem] p-10 shadow-2xl shadow-slate-200 border border-slate-100 animate-in fade-in zoom-in">
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <p className="text-slate-500 mt-4 font-medium text-sm">La certitude de l'expertise.</p>
          
          {companyName && (
             <div className="mt-4 flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full border border-slate-100">
                <Building2 size={12} className="text-indigo-600" />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Espace : {companyName}</span>
             </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-50 rounded-2xl py-4 pl-12 pr-4 font-bold outline-none border border-transparent focus:border-indigo-100"
        ...
        <p className="mt-8 text-center text-[10px] text-slate-300 font-bold uppercase tracking-widest leading-relaxed">
          Propriété de {companyName || 'AutoExpert'} • Accès Sécurisé
        </p>
      </div>
    </div>
  );
