...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, user, onLogout, isAdmin, companyName, appName, onUpdateUser }) => {
  const [isLogoutModalOpen, setIsLogoutModalOpen] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Définition des items de navigation en fonction du rôle
  const navItems = [];
  
  if (isAdmin) {
    navItems.push(
      { id: 'dashboard', icon: LayoutDashboard, label: 'Tableau de Bord' },
      { id: 'scanner', icon: ScanLine, label: 'Nouvelle Expertise' },
      { id: 'history', icon: History, label: 'Registre Parc' },
      { id: 'chat', icon: MessageSquare, label: 'IA Expert' },
      { id: 'config', icon: Settings, label: 'Configuration' }
    );
  } else {
    // Un agent a accès au Scanner et à l'Inventaire (pour l'export CSV)
    navItems.push(
      { id: 'scanner', icon: ScanLine, label: 'Expertise' },
      { id: 'history', icon: History, label: 'Registre' }
    );
  }
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Limite de taille simple (ex: 2Mo)
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 glass border-t border-slate-100 px-6 py-4 flex justify-around items-center z-50 safe-pb shadow-[0_-4px_20px_rgba(0,0,0,0.03)]">
          {navItems.map((item) => (
            ...
            >
              ...
              <span className={`text-[9px] font-black uppercase tracking-widest ${activeTab === item.id ? 'opacity-100' : 'opacity-60'}`}>{item.label.split(' ')[0]}</span>
            </button>
          ))}
        </nav>
      </main>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full relative z-10 shadow-2xl border border-slate-100 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-2xl font-black text-slate-900 tracking-tight italic mb-2">Déconnexion ?</h3>
